...
/**Modefied from {@link com.badlogic.gdx.graphics.g3d.utils.ModelBuilder},
 * with builders of MeshBuilder changed to XMeshBuilder.
 * As it's fields are privative, this class is a basically copied source.
 * 
 * @author Odys Zhou
 *
 */
public class XModelBuilder {
	...
	private Array<XMeshBuilder> builders = new Array<XMeshBuilder>();

	// private Matrix4 tmpTransform = new Matrix4();

	private XMeshBuilder getBuilder (final VertexAttributes attributes) {
		for (final XMeshBuilder mb : builders)
			if (mb.getAttributes().equals(attributes) && mb.lastIndex() < Short.MAX_VALUE / 2) return mb;
		final XMeshBuilder result = new XMeshBuilder();
	...
	public MeshPartBuilder part (final String id, int primitiveType, final VertexAttributes attributes, final Material material) {
		final XMeshBuilder builder = getBuilder(attributes);
	...
	public MeshPartBuilder part (final String id, int primitiveType, final long attributes, final Material material) {
		return part(id, primitiveType, XMeshBuilder.createAttributes(attributes), material);
	}
//
