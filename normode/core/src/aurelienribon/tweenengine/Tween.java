...
public final class Tween extends BaseTween<Tween> {
	 ...
	 *
	 * @param target The target object of the interpolation.
	 * @param tweenType @removed The desired type of interpolation.
	 * @return The generated Tween.
	 */
	public static Tween set(Object target, int tweenType) {
		Tween tween = pool.get();
		tween.setup(target, 0);
		tween.ease(Quad.INOUT);
		return tween;
	}
	...
	private void setup(Object target, float duration) {
	...
	public Tween build() {
		...
		if (accessor != null) combinedAttrsCnt = accessor.getValues(target, accessorBuffer);
	...
	protected void initializeOverride() {
		...
		// Odys: both MutableFloat & MutableInteger.getVules don't use 'type'
		accessor.getValues(target, startValues);
	...
	protected void updateOverride(int step, int lastStep, boolean isIterationStep, float delta) {
		...
		if (!isIterationStep && step > lastStep) {
			accessor.setValues(target, isReverse(lastStep) ? startValues : targetValues);
			return;
		}

		if (!isIterationStep && step < lastStep) {
			accessor.setValues(target, isReverse(lastStep) ? targetValues : startValues);
			return;
		}
		...
		if (duration < 0.00000000001f && delta > -0.00000000001f) {
			accessor.setValues(target, isReverse(step) ? targetValues : startValues);
			return;
		}

		if (duration < 0.00000000001f && delta < 0.00000000001f) {
			accessor.setValues(target, isReverse(step) ? startValues : targetValues);
			return;
		}
		...
		accessor.setValues(target, accessorBuffer);
	}
	...
	protected void forceStartValues() {
		if (target == null) return;
		accessor.setValues(target, startValues);
	}

	@Override
	protected void forceEndValues() {
		if (target == null) return;
		accessor.setValues(target, targetValues);
	}
